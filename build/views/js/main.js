...
var resizePizzas = function(size) {
  ...
  //Change notes
  //====================================================================
  //Deleted dx calculation cause of extra work, that it was doing to calculated new size for pizza.
  //Old algorithm worked next way:
  //In cycle, it calculated new size as fraction of parent container's width.
  //Then it measured difference between previous pizza element size and new one as dx.
  //Then it added calculated dx to old size.
  //Algorithm was rewritten due to next reasons:
  //This is too irrational, because new size does not change from iteration to iteration.
  //And there is no point in calculating difference - we can simply set calculated size as width.
  //Also, we don't need to calculate fractions - we can just set new width in percents.
  //If we don't need to calculate fractions, we don't need to get pizza elements container width
  //which saves us layout step
...
function updatePositions() {
  ...
  var phases = [];
  for (var i = 0; i < 5; ++i)
    phases[i] = Math.sin((scrollTop / 1250) + i) * 100;

  //changed property from left to transform, cause
  //according to http://csstriggers.com/ transform doesn't call layout and paint
  //and  left triggers layout, paint and composite, which costs a lot
